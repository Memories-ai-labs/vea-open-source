...
from src.pipelines.qualityAnalysis.schema import QualityAssessmentResult, EvaluationScore, BrandSafetyScores
...
class QuizingEvaluation:
    def __init__(self, benchmark_root: str = "benchmark_outputs"):
    ...
    def _discover_models_for_movie(self, movie_name: str) -> List[str]:
        ...
        print(f"[DEBUG] Found models for '{movie_name}': {models}")
        return models

    def _get_index_path(self, movie_name: str, model_name: str) -> Path:
        model_path = self.benchmark_root / movie_name / model_name
        if model_name == "ours":
            path = model_path / "media_indexing.json"
        else:
            path = model_path / "naive_combined_summary.txt"
        if not path.exists():
            print(f"[WARN] Missing index for {movie_name} | {model_name}: {path}")
            return None
        print(f"[DEBUG] Found index file for {movie_name} | {model_name}: {path}")
        return path
    ...
    def _format_prompt(self, question: str, context: str) -> str:
        return (
            ...
            f"Context:\n{context}\n\n"
            f"Question: {question}\n"
            f"Give your best answer as if you're summarizing for a viewer. Be specific. you are not allowed to use any external information or knowledge. you should answer the question strictly based on the available context\n\n"
        )

    def _scoring_prompt(self, predicted: str, reference: str) -> str:
        return (
            "You are evaluating the quality of an AI-generated answer to a question about a video or movie.\n\n"
            f"Ground truth:\n{reference}\n\n"
            f"AI's answer:\n{predicted}\n\n"
            "Rate the AI's answer on the following three criteria, using a score from 1 (very poor) to 5 (excellent):\n\n"
            "- factual_correctness: Is the content factually accurate compared to the ground truth? Are the characters, events, and descriptions correct?\n"
            "- timestamp_accuracy: If the question asks about specific times or event order, does the answer preserve the correct sequence and causal relationships? "
            "If no timestamps are involved, assess whether the answer accurately captures the **order of events** and avoids logical confusion (e.g. mixing cause and effect).\n"
            "- detail_completeness: Does the answer include all relevant and important details given in the ground truth, without being vague or omitting key points?\n\n"
            "Respond in the following JSON format:\n"
            "{\n"
            "  \"factual_correctness\": 5,\n"
            "  \"timestamp_accuracy\": 4,\n"
            "  \"detail_completeness\": 5\n"
            "}"
        )
    ...
    def run(self) -> List[dict]:
        ...
        for i, row in enumerate(self.questions):
            ...
            for model in available_models:
                print(f"[MODEL] Evaluating model: {model}")
                index_path = self._get_index_path(movie, model)
                ...
                try:
                    ...
                    scoring_prompt = self._scoring_prompt(predicted_answer, answer)
                    # print(f"[SCORING PROMPT -> Gemini]\n{scoring_prompt}\n")
                    scores_obj = self.llm.LLM_request([scoring_prompt], EvaluationScore)
                    scores = scores_obj
                    ...
                    print(f"[ERROR] Exception during evaluation of {movie} | {model}: {e}")
        print(f"\n[COMPLETE] Evaluated {len(results)} model-question pairs.")
        return results
